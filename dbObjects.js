...
Users.prototype.getFossilItem = async function(name) {
	return fossilName = await CurrencyShop.findOne({ where: { name: { [Op.like]: name } } });
};
...
Users.prototype.addHave = async function(item) {
	...
	return UserItems.create({ user_id: this.user_id, item_id: item.id, amount_have: 1, amount_need: 0 });
};
...
Users.prototype.addNeed = async function(item) {
	...
	return UserItems.create({ user_id: this.user_id, item_id: item.id, amount_have: 0, amount_need: 1 });
};
